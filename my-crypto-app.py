...
#=============================
    ...
    st.header("🔑 RSA脆弱性デモ ")
    st.warning("公開鍵から秘密鍵を計算で特定します。  同じ鍵長で複数回実行すると、グラフが正常に表示されなくなります。")
...
#=============================
    ...
    st.warning("""
短い鍵を全パターン試して解読する実験です。
同じ鍵長で複数回実行すると、グラフが正常に表示されなくなります。
※ この実験においてAESの鍵長（128/256bit等）は計算結果にほぼ影響しないため、本デモでは標準的な128bitに固定しています。
    """)
        ...
        **総当たり攻撃の仕組み**  
        ...
        if st.button("総当たり開始", type="primary"):
            ...
            if found_key:
                ...
                st.markdown(f"""
これは暗号を解除するための「共通鍵（合言葉）」そのものです。この鍵を使うことで、暗号化されたメッセージを元の読める状態に戻せます。
