...
#=============================
    ...
    target_bits = st.slider("攻撃の難易度 (探索ビット数)", 8, 22, 14, 
                            help="数値を大きくするほど、試行回数が増えて解読に時間がかかります。")
    
    st.caption("※ この実験においてAESの鍵長（128/256bit等）は計算結果にほぼ影響しないため、本デモでは標準的な**128bitに固定**しています。")
