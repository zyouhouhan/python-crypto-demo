            ...
            **全数探索の仕組み**  
            ...
            if st.button("総当たり開始", type="primary"):
                ...
                if found_key:
                    ...
                    st.markdown("これは暗号を解除するための「共通鍵（合言葉）」そのものです。この鍵を使うことで暗号化されたメッセージを元の読める状態に戻せます。")
                    
                    st.success("""
                    右側の数文字:  
                    今回の探索範囲内で特定された**正解のパーツ** 
                    左側の連続する0:  
                    解読時間を短縮するために、あらかじめ鍵の大部分を**「0」に固定**していた箇所
                    """)
                    st.markdown("本来のAESはこの128bitすべてが予測不能なランダムな値であるため、この「力技」で正解にたどり着くことは不可能です。")
                    st.divider()
                    st.session_state['attack_history'].append({
