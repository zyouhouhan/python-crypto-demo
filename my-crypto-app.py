...
1. **運の要素（当たり場所）**:
   総当たり攻撃は、正解を見つけた瞬間に終了します。
   - **運が良いとき**: 100万通りの最初の方で正解が見つかる（一瞬で終わる）。
   - **運が悪いとき**: 最後の方まで正解が見つからない（時間がかかる）。

2. **PCの「ご機嫌」**:
   Pythonが裏側で他の処理をしていたり、メモリの整理（ガベージコレクション）を始めたりすると、計算速度が一時的に落ちることがあります。

**結論**: 
1回ごとの結果にはムラがありますが、**何度も試して「平均」をとると、必ずビット数が多いほど時間は長くなります。** これが暗号の「確率的な安全性」です。
""")
