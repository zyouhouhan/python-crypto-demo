...
def play_video_autoplay(video_path):
    # 動画ファイルをバイナリで読み込んでBase64に変換
    video_file = open(video_path, 'rb')
    video_bytes = video_file.read()
    b64_video = base64.b64encode(video_bytes).decode()
    
    # HTMLタグを作成 (muted, autoplay, loop, playsinline が必須)
    video_html = f'''
    ...
    '''
    st.components.v1.html(video_html, height=400) # heightは動画に合わせて調整

# 実行部分
video_file_path = Path(__file__).parent / "assets" / "crypto_demo.mp4"
if video_file_path.exists():
    play_video_autoplay(str(video_file_path))
# --- 解説部分（3分割） ---
