...
# 動画のURL
video_url = "https://github.com/zyouhouhan/python-crypto-demo/blob/main/demo.mp4?raw=true"

# 表示用の関数（ボタンもHTMLの中に含める）
def play_video_with_button(url):
    video_html = f'''
        <div style="width: 100%; text-align: center;">
            <video id="myVideo" width="100%" autoplay loop muted playsinline style="border-radius: 10px; margin-bottom: 10px;">
                <source src="{url}" type="video/mp4">
                Your browser does not support the video tag.
            </video>
            <br>
            <button onclick="resetVideo()" style="
    ...
    '''
    # 動画(約360px) + ボタン(約50px) + 余白 を考えて高さを 420 くらいに絞る
    st.components.v1.html(video_html, height=380)

# 実行
play_video_with_button(video_url)
...
#===================
    ...
    with st.expander("""AES暗号における共通鍵の役割  
   (かなり複雑な説明です)"""):
        st.write("""まず、AES暗号は元のメッセージをいくつもの工程を通して、まったく予想のつかない暗号文へと変換します。
    そして復号（元に戻す作業）は、暗号化で行った工程をすべて「逆の順番」で行います。
    
    各工程の内容（アルゴリズム）は世界中に公開されていますが、それでも解読できない理由は**「共通鍵」**にあります。
    """)
        st.info("""
    共通鍵の役割
    各工程には、鍵がないと動かない「仕掛け」が組み込まれています。
    
    **1. 計算の向きを決める**:  
    各工程でデータをどう入れ替えるかは、鍵のデータによって決まります。鍵が1ビットでも違えば、計算結果はまったく別のものになります。
    **2. 逆再生の指示書**:  
    共通鍵は「どの手順でどう混ぜたか」を記録したレシピのようなものです。これがあるからこそ、複雑に混ざったデータを正確に逆再生して元に戻せます。
    
    つまり、「やり方」は全員が知っていても、手元に「本物の鍵」がない限り、逆算の計算式を解くことができないようになっているのです。
    """)
...
#=============================
    ...
    st.header("🔑 RSA脆弱性デモ ")
    st.warning("""
    **試し割り**で公開鍵から秘密鍵を特定します。  
    同じ鍵長で複数回実行すると、グラフが正常に表示されなくなります。ご注意ください。
               """)
    
    with st.expander("ℹ️ 試し割りとは"):
        st.write("""
        ある数字（公開鍵）に対して、小さい数字から順番に割り算をしていき、余りが 0 になる数字（素因数）を見つけ出す手法です。
        """)
    ...
    with col_set1:
        weak_bits = st.number_input(
            "RSA鍵長 (推奨: 16〜30)", 
            min_value=8, 
            max_value=32, 
            value=16,
            help="鍵長とは、ビット(bit)数と同じ意味です。"
        )
    with col_set2:
        st.write("")
...
#=============================
    ...
    同じ鍵長で複数回実行すると、グラフが正常に表示されなくなります。ご注意ください。
    """)
    st.caption("※ この実験においてAESの鍵長（128/256bit等）は計算結果にほぼ影響しないため、本デモでは標準的な128bitに固定しています。")
    
    with st.expander("ℹ️ 全数探索とは"):
        st.write("""
        鍵、つまりはパスワードをしらみつぶしに試して正解を探すというような手法です。
        自転車などのダイヤル錠で、000から999まで全部試す作業をイメージしてください。
        """)
    ...
    with st.expander("ℹ️ 探索範囲とは"):
        ...
        そのため、このデモでは通常よりも狭い範囲（8~22bit）に鍵を隠すことで短時間で正解を見つけられるようにしています。
        """)
        
    if st.button("決定", type="primary"):
        st.session_state['aes_step1_done'] = True

    if st.session_state.get('aes_step1_done'):
        st.divider()
        # STEP 2: 生成
        st.subheader("STEP2: ターゲットの生成")
    ...
    if 'aes_demo_target' in st.session_state:
        ...
        st.info(f"""
        **今から攻撃（解読）される暗号文:** `{tgt['cipher'].hex()}`
        
        **全数探索の仕組み**  
        この攻撃は、攻撃者が設定されたbit数（$2^{{{tgt['bits']}}}$ 通り = {1 << tgt['bits']:,} 通り）の予測の共通鍵をすべて生成し、
        復号すると元のメッセージになる鍵を探し出すというものです。
        """)
        st.caption("※探索範囲を限定した学習用デモのため、短時間で完了します。")
        ...
        if st.button("総当たり開始", type="primary"):
            ...
            if found_key:
                ...
                st.markdown("これは暗号を解除するための「共通鍵（合言葉）」そのものです。")
                ...
                **解説：**
                ...
                探索範囲内で特定された**正解のパーツ**
                - **左側の連続する0**:  
                解読時間短縮のために、あらかじめ鍵の大部分を**0に固定**していた箇所
                
                本来のAESはこの128bitすべてがランダムなため、全数探索で正解にたどり着くことは不可能です。
                """)
...
#=============================
    ...
    if not st.session_state.get('attack_history') or len(st.session_state['attack_history']) == 0:
        ...
        with st.expander("グラフが階段状にならない場合の理由"):
            ...
            st.info("""
    1. **運の要素**  
試し割りも全数探索も、正解を見つけた瞬間に終了します。そのため全パターンの最初の方で正解が見つかる場合や、最後の方まで見つからず時間がかかってしまう場合があります。
...
Pythonが裏側で他のアプリの処理をしていたり、メモリの整理（ガベージコレクション）を実行したりすると、計算速度が一時的に落ちることがあります。
   """)
        st.warning("""
        【**TRY!**】  
        RSA脆弱性デモの鍵長、AES脆弱性デモの探索範囲のビット数を揃えてグラフを表示してみましょう！
        """)
        with st.expander("TRY!で生成されるグラフの解説"):
            st.write("""
            グラフの結果から、同じnビットの設定でもRSAの方が圧倒的に速く解読されることがわかりました。
            これは、RSAの試し割りが「数字のルート ($\sqrt{n}$)」まで計算すれば良いという数学的性質を持っているのに対し、AESは $2^n$ 通りの全パターンの共通鍵を愚直に試す必要があるためです。
            つまり、**「同じビット数ならAESの方が圧倒的に解読が難しく、安全である」**ということが実証されました。
            """)
